...
BodyParser.prototype = {
    ...
    parse: function (request, callback) {
        ...
        if (this.maxContentLength !== undefined && this.maxContentLength < content_length) {
            log.e('[%s] Request entity too large (content_type=%s, content_length=%d).', request.peerName, content_type, content_length);
            ...
            log.e('[%s] Unsupported Content-Type "%s". (content_length=%d).', request.peerName, content_type, content_length);
            if (request.getResponse())
